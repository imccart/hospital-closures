...
The capacity effects are the most precisely estimated results in our analysis. They are robust across two distinct control group constructions, the state-timing design and an eligibility-restricted alternative that relaxes the state-timing requirement entirely, and are confirmed by interactive fixed effects models that accommodate unbalanced panels natively (reported in the supplemental appendix). Permutation inference produces $p$-values broadly consistent with the jackknife-based confidence intervals. The heterogeneity analysis further demonstrates that these reductions are pervasive across hospital ownership types, levels of geographic isolation, and pre-treatment financial health, reinforcing the interpretation that the effects reflect the program's design rather than selection by a particular subset of hospitals. Where capacity outcomes exhibit differential pre-trends at $t = -1$, the anticipation analysis suggests this reflects the mechanism itself: hospitals must actively restructure operations to meet eligibility requirements before formal designation, and redefining treatment to begin one year earlier both eliminates the pre-trend concern and strengthens the estimated effects.

The financial decomposition provides a useful lens for interpreting the null margin result but rests on weaker identification: the revenue and expense components exhibit differential pre-trends in some cohorts. We view this decomposition as suggestive rather than definitive. The null margin result itself does not depend on the decomposition and is robust across estimators, control group constructions, and pre-period lengths. Financial outcomes more broadly rely on balanced panels of 34--43 treated hospitals because HCRIS coverage ramps up gradually through the late 1990s; interactive fixed effects estimates that retain the full sample yield qualitatively similar conclusions. The current ratio is the one outcome for which SDID and Callaway--Sant'Anna produce opposite-signed estimates; this divergence attenuates substantially in the eligibility-restricted design, consistent with sensitivity to balanced-panel composition rather than a genuine effect. Our identification period also coincides with several concurrent Medicare payment reforms, including the Balanced Budget Refinement Act of 1999 and the Benefits Improvement and Protection Act of 2000, that affected hospital reimbursement more broadly. While our stacked design compares hospitals across states within narrow event windows, we cannot fully separate the effects of CAH designation from differential exposure to these concurrent changes.
