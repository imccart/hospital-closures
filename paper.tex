...
\label{eq:cah_profit}
\end{equation}
where $\alpha>0$ reflects the CAH reimbursement bump. The first implication of Eq.~\eqref{eq:cah_profit} is a direct effect on hospital margins. With higher reimbursement for Medicare patients, net margins rise under CAH status provided the incremental payment exceeds any cost increase required to comply with designation rules. This channel is central to our first set of empirical tests. The notation $p_c(\delta)$ allows for the possibility that commercial prices respond to state-level CAH availability---for example, if reduced capacity among converting hospitals affects bargaining leverage with private insurers---but we do not test this channel directly.
...
We begin by examining how CAH designation affects hospital operating margins, the most immediate outcome of the reimbursement change in Section~\ref{sec:background}. In this analysis, treatment is defined at the hospital level: a hospital is considered treated once it receives CAH designation, and the unit of observation is the hospital-year. The sample is restricted to facilities plausibly eligible for designation (i.e., small rural hospitals consistent with program criteria), excluding hospitals with missing financial data. This setup isolates the direct effect of adopting CAH status on hospital margins.
...
Figure~\ref{fig:sdid-financial} reports SDID estimates of CAH designation on operating margin, the current ratio, and the components of operating margin---net patient revenue per bed and total operating expenses per bed. Contrary to the prediction that cost-based reimbursement should raise margins, the pooled SDID ATT for operating margin is $-0.010$ (95\% CI $[-0.05, 0.02]$), centered near zero and statistically insignificant. The CS estimate is similarly null ($0.001$, 95\% CI $[-0.05, 0.05]$). This finding is surprising given the direct reimbursement channel in Eq.~\eqref{eq:cah_profit}, but decomposing the margin into its components clarifies the underlying mechanism.
...
\paragraph{\textit{Identification and robustness.}}
Our primary identification strategy exploits the staggered rollout of state-level CAH availability, comparing hospitals that convert in early-adopting states to otherwise similar hospitals in states where the program was not yet available. This design limits reliance on within-state comparisons between converters and contemporaneous non-converters, whose decision not to convert may reflect unobserved hospital-level selection. However, it cannot fully eliminate the possibility that hospitals selecting into CAH status differ from controls along unobservable dimensions. The alternative control group analysis in Section~\ref{sec:robustness_elig} provides reassurance on this front: relaxing the state-timing restriction and widening the control pool to all non-CAH hospitals with 50 or fewer beds yields qualitatively similar results across outcomes, with capacity effects that are often larger in magnitude. This consistency across two distinct control group constructions---one emphasizing exogeneity of the state-level policy shock, the other emphasizing breadth and comparability of the control pool---strengthens the case that our estimates reflect genuine treatment effects rather than artifacts of a particular sample selection.

\paragraph{\textit{Pre-trends and the revenue/expense decomposition.}}
While capacity and organizational outcomes generally satisfy standard pre-trend diagnostics, the revenue and expense decomposition exhibits differential pre-trends in some cohorts. We present this decomposition as suggestive rather than definitive evidence. The stability of SDID point estimates across pre-period lengths of 2--5 years, documented in the supplemental appendix, provides some reassurance that the estimates are not mechanically driven by fitting differential trends, but we cannot rule out the possibility that stable pre-existing differences between treated and control hospitals contribute to the estimated effects. The null margin result itself does not rely on the decomposition and is robust across estimators, control group constructions, and pre-period lengths.

\paragraph{\textit{Small treated samples.}}
Financial outcomes rely on balanced panels of 34--43 treated hospitals, depending on the pre-period length. These sample sizes arise because HCRIS financial data coverage ramps up gradually through the late 1990s, and the SDID estimator requires each unit to be observed in every period of the event window. While jackknife standard errors provide valid inference asymptotically, their finite-sample properties are less well-characterized with very few treated units. We report cohort-specific estimates throughout to allow readers to assess the stability and fragility of the aggregate results. The pre-period sensitivity analysis further demonstrates that the key findings---null margins, proportional contraction in revenue and expenses, and stable capacity effects---are not driven by a particular subset of hospitals that happen to have unusually long financial histories.

\paragraph{\textit{SDID and CS divergence.}}
The current ratio is the one outcome for which our two estimators produce qualitatively different results. The SDID estimate implies a decline in liquidity following CAH designation, while the Callaway--Sant'Anna estimate implies an increase. This divergence is substantially attenuated in the eligibility-restricted design, where the SDID current ratio estimate is $-0.121$ (versus $-0.709$ in the state-timing design) and the CS estimate is $0.500$ (versus $1.816$). The attenuation is consistent with balanced-panel composition effects: when the SDID balanced panel retains only a small and potentially unrepresentative subset of treated units, the resulting estimates may diverge from those obtained under the full unbalanced sample. We therefore do not draw strong conclusions about the direction of liquidity effects from either estimator.

\paragraph{\textit{Policy implications.}}
CAH designation preserves hospital \textit{presence} but not pre-designation \textit{capacity}. This distinction has direct implications for evaluating the program's effectiveness. As our back-of-the-envelope calculations in Section~\ref{sec:net_capacity} demonstrate, the break-even conversion rate---the share of hospitals that would need to convert for avoided closures to offset within-hospital downsizing---is approximately 6\% for beds and 4\% for inpatient days. These thresholds are well below observed conversion rates in states that adopted the program, implying that the net effect of CAH designation on aggregate inpatient capacity is negative at the scale at which the program has operated.

These findings are relevant to ongoing debates about designation-based hospital support. The Rural Emergency Hospital (REH) designation, introduced in 2023, represents a further point along the survival-scale tradeoff: REH status eliminates inpatient services entirely in exchange for enhanced outpatient reimbursement. Our results for CAH---where the capacity constraints are less severe---suggest that even modest eligibility restrictions can generate meaningful reductions in service scope. Whether the access value of maintaining a hospital presence (e.g., emergency care, outpatient services, local employment) outweighs the diminished inpatient capacity is ultimately a welfare question that our analysis cannot resolve without patient-level data on utilization, health outcomes, and travel burden.

\paragraph{\textit{Future directions.}}
Several extensions would sharpen the analysis. First, heterogeneity analysis along dimensions of hospital ownership (government versus nonprofit), initial bed size ($\leq 25$ versus 26--50), and geographic isolation could clarify which types of hospitals experience the largest capacity reductions and whether the survival benefit is concentrated among the most financially distressed facilities. Second, alternative estimators designed for unbalanced panels, such as the generalized synthetic control method \citep{xu2017} or the fixed-effects counterfactual estimator, would allow the full sample to contribute to financial outcome estimates without the attrition imposed by balanced-panel requirements. Third, downstream analyses linking CAH designation to patient-level outcomes---including emergency department utilization, inpatient mortality, and travel distances---would directly inform the welfare implications of the survival-scale tradeoff documented here.
