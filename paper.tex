...
We implement \eqref{eq:net_beds} using our pooled SDID estimates: $\Delta C=-0.483$ closures per 100 hospitals and $\Delta B=-4.081$ beds per converting hospital. For a back-of-the-envelope conversion intensity we set $\rho=0.30$. To proxy $\bar B_{\text{close}}$, we use the mean pre-CAH bed size from Table~\ref{tab:summary-by-cah}, $\bar B_{\text{close}}\approx 54$. Dividing \eqref{eq:net_beds} by $N$ yields the net effect per hospital:
\begin{equation*}
